...
| **Versus RaBitQ and OPQ at matched bytes on public data:** wins at the low-byte end on low-dimensional data, ties once the rerank saturates, and loses only where its own kernel wrapped or at one 132-byte Wikipedia point | `embedding_beats_rabitq_ties_opq` | Yes — preregistered six-arm campaign, scored by rule; verbatim scorer reports committed | GloVe-100 / deep-image-96 / NYTimes-256 / DBpedia ada-002 1M / DBpedia 3-large 1M / Wikipedia-1024 10M | [`benchmarks/RESULTS_rabitq_public.md`](benchmarks/RESULTS_rabitq_public.md) · [`docs/PREREG_rabitq_public.md`](docs/PREREG_rabitq_public.md) · [`00_canonical_sota_embedding.ipynb`](notebooks/claims/00_canonical_sota_embedding.ipynb) | CPU | **reproducible** (registered verdicts: C1 beats RaBitQ MIXED 15/47/10, C2 ties OPQ MIXED 6/12/4; 21 of 540 cells not run, Amendment 4) |
