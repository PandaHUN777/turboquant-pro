...
|---|:--:|---|
| **Versus RaBitQ and OPQ at matched bytes on public data:** tq-pro wins at the low-byte end on low-dimensional data, ties once the rerank saturates, and loses only where its own kernel wrapped or at one 132-byte Wikipedia point (ledger: `embedding_beats_rabitq_ties_opq`, *reproducible*; registered verdicts C1 beats RaBitQ MIXED, C2 ties OPQ MIXED, from a preregistered six-arm campaign). The private-LaBSE headline, where it beat RaBitQ outright, is `embedding_labse_32x_headline`, *reported*. Index build 4–20× faster than OPQ. | L1 (committed, preregistered, scored by rule) | **[`benchmarks/RESULTS_rabitq_public.md`](../benchmarks/RESULTS_rabitq_public.md)** — the scored record and the 21 cells not run; [`00_canonical_sota_embedding.ipynb`](../notebooks/claims/00_canonical_sota_embedding.ipynb) is the one-*Run all* form on public GloVe. |
